<?php
	...
	$sql = "SELECT sl_service_id FROM related_device_sl WHERE device_id=" . $id;
    ...
    foreach ($serviceIDS as $service_id) {
		$sql = "SELECT id, name, category FROM sl_service WHERE id=" . $service_id;
		$result = $conn->query($sql);
        $resultsNumber = $result->num_rows;
		if ($resultsNumber == 1) {
